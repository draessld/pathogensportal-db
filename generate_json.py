"""
Generuje JSON data soubory pro Chart.js grafy.

Vstup:  $DATA_DIR   (výchozí ./data)          — stažené CSV ze scraperů
Výstup: $OUTPUT_DIR (výchozí ./output/charts) — *.json pro Chart.js

Přenositelnost: cesty jsou konfigurovatelné přes proměnné prostředí, takže tenhle
repozitář jde použít samostatně i jako git submodule cizího projektu. Např. portál:
...
DATA_IN = Path(os.getenv("DATA_DIR", ROOT / "data"))
DATA_OUT = Path(os.getenv("OUTPUT_DIR", ROOT / "output" / "charts"))
